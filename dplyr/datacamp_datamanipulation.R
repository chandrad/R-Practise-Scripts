### DPLYR place####
#1,Load the dplyr and hflights package## Welcome to the interactive exercises part of your dplyr course. ## Here you will learn the ins and outs of working with dplyr. dplyr is an R package, ## a collection of functions and data sets that enhance the R language.
...
###INSRTUCTIONS#####Load the dplyr package.##Load the hflights package. A variable called hflights will become available, a data.frame representing the data set.##Use both head() and summary() on the hflights data frame to get to know the data. Can you guess the meaning of all variables?
...
head(hflights); summary(hflights)
...
##INSTRUCTIONS###
#Convert hflights (which is a data.frame) into a tbl, also named hflights.#Display the new hflights in your console window. Notice the easy-to-read layout.#To see how tbls behave like data.frames, save the UniqueCarrier column of hflights as an object named carriers, #using standard R syntax only.
...
#You can create a lookup table with a named vector. When you subset the lookup table with a #character string (like the character strings in UniqueCarrier), #R will return the values of the lookup table that correspond to the names in the character string. #To see how this works, run following code in the console:  #two <- c("AA", "AS")#lut <- c("AA" = "American", #         "AS" = "Alaska", #         "B6" = "JetBlue")#two <- lut[two]#two
##INSTRUCTIONS##
## Use lut to translate the UniqueCarrier column of hflights. ## Assign the recoded vector to the UniqueCarrier column again.## It's rather hard to assess whether your solution is actually the right one, ## since the UniqueCarrier variable does not appear when you display hflights. ## Use the glimpse() function to inspect the raw values of the variable

# Both the dplyr and hflights packages are loaded into workspacelut <- c("AA" = "American", "AS" = "Alaska", "B6" = "JetBlue", "CO" = "Continental",          "DL" = "Delta", "OO" = "SkyWest", "UA" = "United", "US" = "US_Airways",          "WN" = "Southwest", "EV" = "Atlantic_Southeast", "F9" = "Frontier",          "FL" = "AirTran", "MQ" = "American_Eagle", "XE" = "ExpressJet", "YV" = "Mesa")
# Use lut to translate the UniqueCarrier column to hflightshflights$UniqueCarrier <- lut[hflights$UniqueCarrier]
glimpse(hflights)
# Both the dplyr and hflights packages are loaded into workspacelut <- c("AA" = "American", "AS" = "Alaska", "B6" = "JetBlue", "CO" = "Continental",          "DL" = "Delta", "OO" = "SkyWest", "UA" = "United", "US" = "US_Airways",          "WN" = "Southwest", "EV" = "Atlantic_Southeast", "F9" = "Frontier",          "FL" = "AirTran", "MQ" = "American_Eagle", "XE" = "ExpressJet", "YV" = "Mesa")
# Use lut to translate the UniqueCarrier column of hflightshflights$Carrier <- lut[hflights$UniqueCarrier]
# Inspect the resulting raw values of your variablesglimpse(hflights)
## 4. Changing labels of hflights, part 2 of 2
## Using the concepts covered in the previous exercise, you are challenged to complete a similar ## exercise with minimal additional help. This time, you are to change the labels in the CancellationCode column. ## This column lists reasons why a flight was cancelled using a non-informative alphabetical code. Execute
# unique(hflights$CancellationCode)# in the console to have a look.
# The actual meaning of the codes is the following:  #"A" stands for "carrier",#"B" stands for "weather",#"C" stands for "FFA" and#"D" denotes "security".#"E" denotes that the flight was "not cancelled".
###INSTRUCTIONS###Using the information above, create a lookup table lut that is able to convert #the alphabetical codes into the more meaningful strings.#Use lut to change the labels of the CancellationCode column of hflights.#As before, check your results by glimpsing at them.
# The hflights tbl you built in the previous exercise is available in the workspace.
# Build the lookup table: lutlut <- c("A" = "carrier", "B" = "weather", "C" = "FFA", "D" = "security", "E" = "not cancelled")
# Use the lookup table to create a vector of code labels. Assign the vector to the CancellationCode column of hflightshflights$Code <- lut[hflights$CancellationCode]
# Inspect the resulting raw values of your variablesglimpse(hflights)
## Section dplyr has grammar of 5 verbs functions of basic data manipulation## 1. select -  removes `columns`## 2. filter  - removes `rows`## 3. arrange - reorders the `rows` in the data set## 4. mutate - Creates new `columns` with new values## 5. summarize - calculates summary statistics
## select & mutate manipulate the variables in the dataset## filter & arrange- manipulate the observations## summarize - summarize the groups of observations
## tidy data. (tidyr)
select(hflights, ActualElapsedTime, AirTime, ArrDelay, DepDelay)

## 5. The five verbs and their meaning
# The dplyr package contains five key data manipulation functions, also called verbs:  # select(), which returns a subset of the columns,# filter(), that is able to return a subset of the rows,# arrange(), that reorders the rows according to single or multiple variables,# mutate(), used to add columns from existing data,# summarise(), which reduces each group to a single row by calculating aggregate measures.# If you want to find out more about these functions, consult the documentation by clicking on the functions above. # What order of operations should we use to to find the average value of the ArrDelay (arrival delay) variable # for all American Airline flights in the hflights tbl?
...
## 6. Choosing is not losing! The select verb
...
# select(df, var1, var2)# You can also use : to select a range of variables and - to exclude some variables, # similar to indexing a data.frame with square brackets. # You can use both variable's names as well as integer indexes. # This call selects the four first variables except for the second one of a data frame df:
# select(df, 1:4, -2)# select() does not change the data frame it is called on; you have to explicitly # assign the result of select() to a variable to store the result.
## INSTRUCTIONS
## Use select() to print out a tbl that contains only the columns ## ActualElapsedTime, AirTime, ArrDelay and DepDelay of hflights.
## Print out a tbl with the columns Origin up to and including Cancelled of hflights.## Find the most concise way to select: columns Year up to and including DayOfWeek, columns ArrDelay up # to and including Diverted. # You can examine the order of the variables in hflights with names(hflights) in the console.
# hflights is pre-loaded as a tbl, together with the necessary libraries.glimpse(hflights)# Print out a tbl with the four columns of hflights related to delayselect(hflights, ActualElapsedTime, AirTime, ArrDelay, DepDelay)
# Print out the columns Origin up to Cancelled of hflightsselect(hflights, 14:19)
# Answer to last question: be concise!select(hflights, 1:4, 12:21)
## 7. Helper functions for variable selection
# dplyr comes with a set of helper functions that can help you select groups of variables inside a select() call:#  - starts_with("X"): every name that starts with "X",#  - ends_with("X"): every name that ends with "X",#  - contains("X"): every name that contains "X",#  - matches("X"): every name that matches "X", where "X" can be a regular expression,#  - num_range("x", 1:5): the variables named x01, x02, x03, x04 and x05,#  - one_of(x): every name that appears in x, which should be a character vector.
## Pay attention here: When you refer to columns directly inside select(), you don't use quotes. ## If you use the helper functions, you do use quotes.
### INSTRUCTIONS ###
## Use select() and a helper function to print out a tbl that contains just ArrDelay and DepDelay of hflights.## Use select() and a helper function to print out a tbl that contains just ArrDelay and DepDelay of hflights.## Find the most concise way to return the following columns with select and its helper functions: DepTime, ### ArrTime, ActualElapsedTime, AirTime, ArrDelay, DepDelay. Use only helper functions!
# As usual, hflights is pre-loaded as a tbl, together with the necessary libraries.glimpse(hflights)# Print out a tbl containing just ArrDelay and DepDelayselect(hflights, ends_with("Delay"))
...
# Print out a tbl as described in the third instruction, using only helper functions.select(hflights, contains('Tim'), contains('Del'))
## 8. Comparison to base R## To see the added value of the dplyr package, it is useful to compare its syntax with base R. ## Up to now, you have only considered functionality that is also available without the use of dplyr. ## The elegance and ease-of-use of dplyr is a great plus though.
## INSTRUCTIONS ###
...
ex1r <- hflights[c("TaxiIn", "TaxiOut", "Distance")]ex1d <- select( hflights, TaxiIn, TaxiOut, Distance)
# Finish select call so that ex2d matches ex2rex2r <- hflights[c("Year", "Month", "DayOfWeek", "DepTime", "ArrTime")]ex2d <- select(hflights, 1:6, -3)
# Finish select call so that ex3d matches ex3rex3r <- hflights[c("TailNum", "TaxiIn", "TaxiOut")]ex3d <- select(hflights, starts_with('Ta'))
## mutate - 
h1 <- select(hflights, Year, ArrDelay, DepDelay)h2 <- mutate(h1, loss = ArrDelay - DepDelay)mean(h2$loss, na.rm=TRUE)hist(h2$loss)
## 9. Mutating is creating ###
...
## Take this example that adds a new column, z, which is the element-wise sum of the columns x and y, ## to the data frame df:  ## mutate(df, z = x + y)
### INSTRUCTIONS ##
...
## Print out g3.
# hflights and dplyr are loaded and ready to serve you.glimpse(hflights)# Add the new variable ActualGroundTime to a copy of hflights and save the result as g1.g1 <- mutate (hflights, ActualGroundTime = ActualElapsedTime - AirTime)
# Add the new variable GroundTime to g1. Save the result as g2.g2 <- mutate(g1, GroundTime = TaxiIn + TaxiOut)
# Add the new variable AverageSpeed to g2. Save the result as g3.g3 <- mutate(g2, AverageSpeed = Distance/AirTime * 60)
# Print out g3print(g3)
### 10. Add multiple variables using mutate ###
### So far you've added variables to hflights one at a time, but you can also use mutate() ## to add multiple variables at once. To create more than one variable, ## place a comma between each variable that you define inside mutate().# mutate() even allows you to use a new variable while creating a next variable in the same call. ## In this example, the new variable x is directly reused to create the new variable y:
### INSTRUCTIONS ###
## Adapt the code that builds m1: add a variable loss_ratio, which is the ratio of loss to DepDelay.
## Create a tbl m2 from hflights by adding three variables:## TotalTaxi, which is the sum of TaxiIn and TaxiOut;## ActualGroundTime, which is the difference of ActualElapsedTime and AirTime;## Diff, the difference between the two newly created variables. This column should be zero for all observations!
# hflights and dplyr are ready, are you?
# Add a second variable loss_ratio to the dataset: m1m1 <- mutate(hflights, loss = ArrDelay - DepDelay, loss_ratio = loss/DepDelay)
# Add the three variables as described in the third instruction: m2m2 <- mutate(hflights, TotalTaxi = TaxiIn + TaxiOut, ActualGroundTime = ActualElapsedTime - AirTime, Diff = ActualGroundTime - TotalTaxi)
### 11. Recap on mutate and select
### As of now, you mastered two of the five data manipulation functions that ### are at the core of dplyr: select() and mutate().
### INSTRUCTIONS ###
## Which statement concerning the following four expressions is correct?### (A) hflights <- select(hflights, -(Year:Month), -(DepTime:Diverted))### (B) select(hflights, starts_with("D"))### (C) select(hflights, -(Year:Month), -(DepTime:Diverted))### (D) hflights <- select(hflights, starts_with("Day"))
## POSSIBLE ANSWERS #### 1. (A) and (C) lead to the same hflights variable and output; (B) and (D) do not.## 2. (A) and (C) lead to the same hflights variable and output, as do (B) and (D).## 3. (A) and (D) lead to the same hflights variable and output; (B) and (C) do not. (ANSWER)## 4. (A) and (D) lead to the same hflights variable and output, as do (B) and (C).
## ANSWER: 3
### Feel free to experiment in the console, hflights is loaded as a tbl.
library(hflights)data(hflights)head(hflights)hflights <- tbl_df(data = (hflights))glimpse(hflights)hflights <- select(hflights, -(Year:Month), -(DepTime:Diverted))select(hflights, starts_with("D"))select(hflights, -(Year:Month), -(DepTime:Diverted))hflights <- select(hflights, starts_with("Day"))glimpse(hflights)
## Section 5 - The third of five vervs: Filter
## Filter: Row equivalent of Select
f1 <- select(hflights, starts_with("Cancel"), DepDelay)f1filter(f1, Cancelled==1)?Comparison
### 13. Logical Operators ###
##R comes with a set of logical operators that you can use inside filter():  ## x < y, TRUE if x is less than y## x <= y, TRUE if x is less than or equal to y## x == y, TRUE if x equals y## x != y, TRUE if x does not equal y## x >= y, TRUE if x is greater than or equal to y## x > y, TRUE if x is greater than y## x %in% c(a, b, c), TRUE if x is in the vector c(a, b, c)## The following example filters df such that only the observations ## for which a is positive, are kept:### filter(df, a > 0)
### INSTRUCTIONS ##### Print out all flights in hflights that traveled 3000 or more miles.## Print out all flights in hflights flown by JetBlue, Southwest, or Delta.## Extract from hflights all flights where taxiing took longer than the actual flight.## Avoid the use of mutate() and do the math directly in the logical expression of filter().
# Both the dplyr and hflights packages are loaded into workspacelut <- c("AA" = "American", "AS" = "Alaska", "B6" = "JetBlue", "CO" = "Continental",          "DL" = "Delta", "OO" = "SkyWest", "UA" = "United", "US" = "US_Airways",          "WN" = "Southwest", "EV" = "Atlantic_Southeast", "F9" = "Frontier",          "FL" = "AirTran", "MQ" = "American_Eagle", "XE" = "ExpressJet", "YV" = "Mesa")
# Use lut to translate the UniqueCarrier column to hflightshflights$UniqueCarrier <- lut[hflights$UniqueCarrier]
# hflights is at your disposal as a tbl, with clean carrier namesglimpse(hflights)# All flights that traveled 3000 miles or morefilter(hflights, Distance >=3000 )
# All flights flown by one of JetBlue, Southwest, or Deltafilter(hflights, UniqueCarrier %in% c('JetBlue','Southwest','Delta'))
# All flights where taxiing took longer than flyingfilter(hflights, TaxiOut + TaxiIn > AirTime)
...
## filter(df, a > 0 & b > 0)## filter(df, a > 0, b > 0)# Next, is.na() will also come in handy. This example keeps the observations in df# for which the variable x is not NA:## filter(df, !is.na(x))
## INSTRUCTIONS ##
## Use R's logical and boolean operators to select just the rows where a flight ## left before 5:00 am (500) or arrived after 10:00 pm (2200).## Print out all of the flights that departed late but arrived ahead of schedule. ## Use DepDelay and ArrDelay for this.## Find all of the flights that were cancelled after being delayed. ## These are flights that were cancelled, while having a DepDelay greater than zero.
glimpse(hflights)
# hflights is at your service as a tbl!
# All flights that departed before 5am or arrived after 10pmfilter(hflights, DepTime < 500 | ArrTime > 2200)
# All flights that departed late but arrived ahead of schedulefilter(hflights, DepDelay > 0 & ArrDelay < 0)
# All flights that were cancelled after being delayedfilter(hflights, Cancelled ==1 & DepDelay > 0)
...
# Select the flights that had JFK as their destination: c1c1 <- filter(hflights, Dest=='JFK')
# Combine the Year, Month and DayofMonth variables to create a Date column: c2c2 <- mutate(c1, Date= paste(Year,Month,DayofMonth, sep='-'))
# Print out a selection of columns of c2print(select(c2, Date, DepTime, ArrTime, TailNum))
...
## POSSIBLE ANSWERS ### 1. 155 flights# 2. 1695 flights# 3. 1784 flights# 4. 6042 flights
c1 <- filter(hflights, DayOfWeek== 6 | DayOfWeek==7)filter(c1, Distance > 1000 , TaxiIn + TaxiOut < 15)## ANSWER: 1. 155 flights
...
### INSTRUCTIONS### Arrange dtc, by departure delays so that the shortest departure delay is # at the top of the data set.
...
# dplyr and the hflights tbl are availableglimpse(hflights)# Definition of dtcdtc <- filter(hflights, Cancelled == 1, !is.na(DepDelay))
# Arrange dtc by departure delaysarrange(dtc, DepDelay)
# Arrange dtc so that cancellation reasons are groupedarrange(dtc, CancellationCode)
# Arrange dtc according to carrier and departure delaysarrange(dtc, UniqueCarrier, DepDelay)
...
# dplyr and the hflights tbl are availableglimpse(hflights)# Arrange according to carrier and decreasing departure delaysarrange(hflights, UniqueCarrier, desc(DepDelay))
# Arrange flights by total delay (normal order).arrange(hflights, DepDelay + ArrDelay)
...
### POSSIBLE ANSWERS ##### 1. First mutate(), then filter(), select() and finally arrange().## 2. First filter(), then mutate(), arrange() and finally select(). ### ANSWER## 3. First arrange(), then mutate(), select() and finally filter().## 4. First select(), then arrange(), filter() and finally mutate().
## ANSwER 2. First filter(), then mutate(), arrange() and finally select().
#### Section 7: Last but not least: summarise ###### summarise uses your data to create a new dataset of summary statistics to describe new data
...
### R comes with many `aggregate` functions:### min, mean, sum, var, sd, max, length, median, IQR### DPLYR supplies many other `aggregate` functions:### first, last, nth, n, n_distinct
...
### INSTRUCTIONS ####### Use summarise() to print out a summary of hflights containing two variables: ### min_dist, the shortest distance flown, and max_dist, the longest distance flown.
...
# Print out a summary with variables min_dist and max_distsummarise(hflights, min_dist=min(Distance), max_dist = max(Distance))
# Print out a summary with variable max_divsummarise(filter(hflights, Diverted==1), max_div = max(Distance))
...
## Print out a summary of temp1 with the following variables (in this order):## earliest: the minimum arrival delay,## average: the average arrival delay,## latest: the longest arrival delay,## sd: the standard deviation for arrival delays.
...
# Remove rows that have NA ArrDelay: temp1temp1 <- filter(hflights,!is.na(ArrDelay))
# Generate summary about ArrDelay column of temp1summarise(temp1, earliest=min(ArrDelay), average=mean(ArrDelay), latest=max(ArrDelay), sd = sd(ArrDelay))
# Keep rows that have no NA TaxiIn and no NA TaxiOut: temp2temp2 <- filter(hflights, !is.na(TaxiIn), !is.na(TaxiOut))
# Print the maximum taxiing difference of temp2 with summarise()summarise(temp2, max_taxi_diff = max(abs(TaxiIn - TaxiOut)))
#### 22. DPLYR aggregate functions
## dplyr provides several helpful aggregate functions of its own, in addition to the ones ## that are already defined in R. These include:
# first(x) - The first element of vector x.# last (x) - The last element of vector x.# nth (x, n) - The `n`th element of vector x.# n() - The number of rows in the data.frame or group of observations that # summarise() describes.# n_distinct(x) - the number of unique values in vector x.
...
### INSTRUCTIONS ###
## Print out a summary of hflights with the following variables:##    n_obs: the total number of observations,##    n_carrier: the total number of carriers,##    n_dest: the total number of destinations,
## aa, a tbl with all flights flown by American Airlines, is already available.
## Print out a summary of aa with the following variables:##      n_flights: the total number of flights (each observation is a flight),##      n_canc: the total number of cancelled flights,##      avg_delay: the average arrival delay of flights whose delay is not NA (na.rm = TRUE)
...
glimpse(hflights)hflights <- tbl_df(hflights::hflights)hflights$UniqueCarrier <- lut[hflights$UniqueCarrier]
# hflights is available with full names for the carriers
# Generate summarizing statistics for hflightssummarise(hflights,          n_obs = n(),          n_carrier = n_distinct(UniqueCarrier),          n_dest = n_distinct(Dest))
# All American Airline flightsaa <- filter(hflights, UniqueCarrier == "American")
# Generate summarizing statistics for aa summarise(aa,          n_flights = n(),          n_canc = sum(Cancelled),          avg_delay = mean(ArrDelay, na.rm = TRUE))
### Section 8 - Chaining your functions: the pipe operator
### pipe operator notation: %>% (magrittr - author Stefan Bache)### What does the pipe do? %>%## %>% (pipe) is an operator that you place in between object and a function### object %>% function(___, arg2, arg3) 
c(1,2,3) %>% sum()c(1,2,3, NA) %>% sum(na.rm = TRUE)hflights$ArrDelay %>% hist(col = 'steelblue', border='white', xlim = c(-50,400))
...
## INSTRUCTIONS ##
## Use `dplyr` functions and the pip operator to transform the following English## sentences into R code:
## Take the hflights set and then ..## Add a variable named `diff` that is the result of subtraction `TaxiIn` from `TaxiOut`, and then## Pick all of the rows whose `diff` value does not equal `NA`, and then## Summarise the data set with a value named `avg` that is the mean `diff` value.
# hflights and dplyr are both loaded and ready to serve you
# Write the 'piped' version of the English sentences.hflights %>%   mutate(diff = TaxiOut - TaxiIn) %>%  filter(!is.na(diff)) %>%  summarise(avg = mean(diff))
...
### INSTRUCTIONS ###
## mutate() the hflights dataset and add two variables:##    RealTime: the actual elapsed time plus 100 minutes (for the overhead that flying involves) and##    mph: calculated as Distance / RealTime * 60, then
## filter() to keep observations that have an mph that is not NA and that is below 70, finally
## summarise() the result by creating four summary variables:##    n_less, the number of observations,##    n_dest, the number of destinations,##    min_dist, the minimum distance and##    max_dist, the maximum distance.
# Chain together mutate(), filter() and summarise()hflights %>%  mutate(RealTime = ActualElapsedTime + 100, mph = Distance / RealTime * 60) %>%  filter(!is.na(mph), mph < 70) %>%  summarise(n_less = n(), n_dest = n_distinct(Dest), min_dist = min(Distance), max_dist = max(Distance))
...
## INSTRUCTIONS ##
## filter() the result of mutate to:##    keep observations that have an mph under 105 or for which ##    Cancelled equals 1 or for which Diverted equals 1.
